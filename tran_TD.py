...
EPOCHS = 1000
GAMMA = 0.2 #since it may take several moves to goal, making gamma high
TURN_BOUNDRY = 2000
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(EPOCHS):
      ...
      for i in range(len(rew)):
        ...
        if rew[i] < -1: #non-terminal state
          for j in range(len(rew)):
            update += FLAGS.learning_rate * (rew[i] + (np.power(GAMMA, j) * act2[i]))
        else: #terminal state
            update = rew[i]
        
        y[0][actt[i]] = update #target output

        feeddict = fill_feed_dict(sta[i].reshape(1,75), y, state_placeholder, action_placeholder)
